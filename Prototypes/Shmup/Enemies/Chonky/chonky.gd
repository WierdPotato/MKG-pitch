...
@onready var collision_polygon_2d: CollisionPolygon2D = $ShootMachine/DetectionArea/CollisionPolygon2D
...
func vector_randomizer() -> void:
	if alive:
		var new_x : float = randf_range(-1, 1)
		var new_y : float = randf_range(-2, 2)
		if smoothing:
